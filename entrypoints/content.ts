...
const INLINE_BLOCKERS = ["onpaste", "oncopy", "oncut", "ondrop", "oncontextmenu", "onselectstart"] as const;
...
class InteractionEnabler {
  ...
  enable(): void {
    ...
    this.observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        if (mutation.target instanceof Element) this.applyElementFeatures(mutation.target);
    ...
    this.observer.observe(document.documentElement, {
      attributes: true,
      attributeFilter: ["autocomplete", "disabled", "draggable", "readonly", ...INLINE_BLOCKERS],
      childList: true,
      subtree: true
    });
  }
  ...
  private applyFeatures(root: ParentNode): void {
    ...
    for (const element of root.querySelectorAll<HTMLElement>("*")) this.applyElementFeatures(element);
  }

  private applyElementFeatures(element: Element): void {
    if (element.hasAttribute("autocomplete")) element.setAttribute("autocomplete", "on");
    if (element.hasAttribute("draggable")) element.setAttribute("draggable", "auto");
